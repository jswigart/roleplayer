...
qreal Box2DWorld::raycast( const QPointF & p0, const QPointF & p1, int filter ) {
	class raycastCB : public b2RayCastCallback {
	public:
		virtual float32 ReportFixture( b2Fixture* fixture, const b2Vec2& point, const b2Vec2& normal, float32 fraction) {
			if ( !(collideWidth & fixture->GetFilterData().categoryBits) ) {
				return -1;
			}

			if ( bestFraction > fraction ) {
				bestFraction = fraction;
			}
			return fraction;
		}

		raycastCB( const int mask ) : 
			collideWidth( mask ), 
			bestFraction( 1.0f ) {}
	...
	raycastCB cb( filter );
	mWorld->RayCast( &cb, b2Vec2(p0.x(),p0.y()), b2Vec2(p1.x(),p1.y()) );

	return cb.bestFraction;
}